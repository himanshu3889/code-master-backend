...
func (s *Store) GetTimelineByProblem(ctx context.Context, problemID int64, limit int) ([]*models.Timeline, *appError.Error) {
	var entries []*models.Timeline
	query := `
		SELECT id, problem_id, submission_id, code_snapshot_id, created_at 
		FROM timeline 
		WHERE problem_id = $1 
		ORDER BY created_at DESC
		LIMIT $2
	`
	if err := s.db.SelectContext(ctx, &entries, query, problemID, limit); err != nil {
...
func (s *Store) GetDetailedTimelineByProblem(ctx context.Context, problemID int64, limit int) ([]*models.DetailedTimeline, *appError.Error) {
	query := `
		SELECT 
			t.id as timeline_id,
	...
	if err := s.db.SelectContext(ctx, &entries, query, problemID, limit); err != nil {
		...
		return nil, appError.NewInternal("Faild to get problem detailed timeline")
	}
